...
async function mostrarListadoDeCines() {
    try {
        ...
        const piezasHtml = ["<br/><h1>Nuestros Cines</h1><br/>"];
        ...
        cajaPrincipal.innerHTML = piezasHtml.join("");
    } catch (error) {
        cajaPrincipal.innerHTML = "Error interno.";
    }
}
...
async function mostrarDetalleDelCine(cineId) {
    try {
        ...
        objRes.forEach(function(docItem) {
            ...
            trozos.push('<h2>' + el.RazonSocial + '</h2>');
            trozos.push('<div class="cine-info"><div class="cine-info datos">');
            trozos.push('<p>' + el.Direccion + ' - ' + el.Detalle + '</p><p>Teléfono: ' + el.Telefonos + '</p><br/>');
            trozos.push('<div class="tabla">');
            ...
            trozos.push(
                ...
                '</div></div><img src="img/cine/' + el.id + '.2.jpg"/><br/><br/>',
                '<h4>Los horarios de cada función están sujetos a cambios sin previo aviso.</h4><br/>',
                '<div class="cine-info peliculas"><div class="tabla">'
            );
            ...
            trozos.push(
                '</div></div></div>',
                '<div><img style="float:left;" src="img/cine/1.3.jpg" alt="Imagen del cine"/>',
                '<span class="tx_gris">Precios de los juegos: desde S/1.00 en todos los Cine Star.<br/>',
                'Horario de atención de juegos es de 12:00 m hasta las 10:30 pm.<br/><br/>',
                'Visitános y diviértete con nosotros.<br/><br/><b>CINESTAR</b>, siempre pensando en tí.</span></div>'
            );
        });
        
        cajaPrincipal.innerHTML = trozos.join("");
    } catch (err) { }
}
...
async function cargarPeliculasEnCartelera() {
    try {
        ...
        const acumuladorHtml = ["<br/><h1>Cartelera</h1><br/>"];

        responseData.forEach(function(docRef) {
            const dat = docRef.data();
            if(Number(dat.id) < 18) {
                acumuladorHtml.push(
                    '<div class="contenido-pelicula"><div class="datos-pelicula">',
                    '<h2>' + dat.Titulo + '</h2><br/><p>' + dat.Sinopsis + '</p><br/>',
                    '<div class="boton-pelicula"><a href="pelicula.html?id=' + dat.id + '">',
                    '<img src="img/varios/btn-mas-info.jpg" width="120" height="30" alt="Info"/></a></div>',
                    '<div class="boton-pelicula"><a href="https://www.youtube.com/v/' + dat.Link + '" target="_blank">',
                    '<img src="img/varios/btn-trailer.jpg" width="120" height="30" alt="Trailer"/></a></div>',
                    '</div><img src="img/pelicula/' + dat.id + '.jpg" width="160" height="226"/><br/><br/></div>'
                );
            }
        });

        cajaPrincipal.innerHTML = acumuladorHtml.join("");
    } catch(err) { }
}
...
async function cargarPeliculasProximas() {
    try {
        ...
        const buffer = ["<br/><h1>Próximos Estrenos</h1><br/>"];
        ...
        docResult.forEach(function(docM) {
            ...
            if(Number(modelo.id) >= 18) {
                buffer.push(
                    '<div class="contenido-pelicula"><div class="datos-pelicula">',
                    '<h2>' + modelo.Titulo + '</h2><br/><p>' + modelo.Sinopsis + '</p><br/>',
                    '<div class="boton-pelicula"><a href="pelicula.html?id=' + modelo.id + '">',
                    '<img src="img/varios/btn-mas-info.jpg" width="120" height="30" alt="mas_info"/></a></div>',
                    '<div class="boton-pelicula"><a href="https://www.youtube.com/v/' + modelo.Link + '" target="_blank">',
                    '<img src="img/varios/btn-trailer.jpg" width="120" height="30" alt="ver_trailer"/></a></div>',
                    '</div><img src="img/pelicula/' + modelo.id + '.jpg" width="160" height="226"/><br/><br/></div>'
                );
            }
        });

        cajaPrincipal.innerHTML = buffer.join("");
    } catch(err) {}
}
...
async function construirMovieDetail(mId) {
    try {
        ...
        const fragmentos = ["<br/><h1>Cartelera</h1><br/>"];

        rs.forEach(function(row) {
            const p = row.data();
            fragmentos.push(
                '<div class="contenido-pelicula"><div class="datos-pelicula">',
                '<h2>' + p.Titulo + '</h2><p>' + p.Sinopsis + '</p><br/>',
                '<div class="tabla">',
                '<div class="fila"><div class="celda-titulo">Título Original :</div><div class="celda">' + p.Titulo + '</div></div>',
                '<div class="fila"><div class="celda-titulo">Estreno :</div><div class="celda">11 de Enero del 2018</div></div>',
                '<div class="fila"><div class="celda-titulo">Género :</div><div class="celda">Aventura / Acción</div></div>',
                '<div class="fila"><div class="celda-titulo">Director :</div><div class="celda">Jake Kasdan</div></div>',
                '<div class="fila"><div class="celda-titulo">Reparto :</div><div class="celda">Dwayne Johnson, Kevin Hart, Jack Black</div></div>',
                '</div></div>',
                '<img src="img/pelicula/' + p.id + '.jpg" width="160" height="226"><br/><br/></div>',
        ...
        cajaPrincipal.innerHTML = fragmentos.join("");
    } catch(e) {}
}
